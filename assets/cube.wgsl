...
fn check_and_spawn(@builtin(global_invocation_id) global_invocation_id: vec3<u32>) {
    var visiable_map: array<bool,20000>;
    var stack: array<vec2<u32>,20000>;

    for (var i: u32 = 0; i < 200; i = i + 1) {
        var stack_index: i32 = 0;
        var stack_length: i32 = 0;
        var is_true: bool = false;
        if visiable_map[i * 100] {
            continue;
        } else {
            visiable_map[i * 100] = true;
        }
        if pic[i][0] == 0 {
            continue;
        }
        ...
        while stack_index <= stack_length {
            ...
            stack_index += 1;
            if x + 1 < 200 && !visiable_map[(x + 1) * 100 + y] && pic[x + 1][y] == now_target {
                stack_length += 1;
                stack[stack_length] = vec2(x + 1, y);
                visiable_map[(x + 1) * 100 + y] = true;
            }
            if y + 1 < 100 && !visiable_map[(x) * 100 + y + 1] && pic[x][y + 1] == now_target {
                stack_length += 1;
                stack[stack_length] = vec2(x, y + 1);
                if y + 1 == 99 {
                    is_true = true;
                }
                visiable_map[(x) * 100 + y + 1] = true;
            }
            if x > 0 && !visiable_map[(x - 1) * 100 + y] && pic[x - 1][y] == now_target {
                stack_length += 1;
                stack[stack_length] = vec2(x - 1, y);
                visiable_map[(x - 1) * 100 + y] = true;
            }
            if y > 0 && !visiable_map[(x) * 100 + y - 1] && pic[x][y - 1] == now_target {
                stack_length += 1;
                stack[stack_length] = vec2(x, y - 1);
                visiable_map[(x) * 100 + y - 1] = true;
            }
